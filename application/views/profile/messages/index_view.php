...
{
	$.ajax({
		url: '<?=base_url()?>my/letters/ajax/mark_read/',
		type: 'POST',
		dataType: 'json',
		data: { id: msg_id },
		success: function(obj) {
			if (obj.result == 'success') {
				$('#msg_'+msg_id).removeClass('letters-list-unread');
				$('#msg_'+msg_id).addClass('letters-list');
				$('#but_'+msg_id).html('');
			}
			else {
				alert(obj.message);
...
{
	$.ajax({
		url: '<?=base_url()?>my/letters/ajax/delete/',
		type: 'POST',
		dataType: 'json',
		data: { id: msg_id },
		success: function(obj) {
...
<div id="menu-letter">
        <a href="#" class="inbox"><p><?=$this->lang->line('letters_inbox')?></p></a>
...
<div class="letters-list<?if($row['is_read'] == 0):?>-unread<?endif;?>" id="msg_<?=$row['msg_id']?>">
   <div class="letters-sender">
    <div id="block-letters-foto"><? if($row['photo'] != '' && $row['from_user_id'] != '0'): ?><img src="<?=base_url()?>profiles/photo/user_<?=$row['from_user_id']?>/<?=$row['photo']?>_101.jpg"><? elseif($row['from_user_id'] == '0'):?><img src="<?=base_url()?>content/img/admin-photo.png"><?else:?><img src="<?=base_url()?>content/img/no-foto-100.png"><?endif;?> </div>
   </div>
   
    <div class="letters-action"><div id="row"><b><?=$this->lang->line('letters_from')?>:</b> <? if($row['from_user_id'] != '0'):?><a href="<?=base_url()?>user<?=$row['from_user_id']?>"><?=$row['name']?></a>, ID: <?=$row['from_user_id']?><? else: ?>Adminitrator<?endif;?></div>
						<div id="row"><b><?=$this->lang->line('letters_subj')?>:</b> <?=$row['subject']?></div>	
                        <a href="<?=base_url()?>my/letters/read/<?=$row['msg_id']?>" class="letters-button"><p><?=$this->lang->line('letters_read')?></p></a>
                        <a href="#." onClick="deleteMessage('<?=$row['msg_id']?>')" class="letters-button"><p><?=$this->lang->line('letters_delete')?></p></a>
                        <? if($row['is_read'] == 0):?><span id="but_<?=$row['msg_id']?>"><a href="#." onClick="markAsRead('<?=$row['msg_id']?>')" class="letters-button"><p><?=$this->lang->line('letters_read2')?></p></a></span><? endif; ?>
